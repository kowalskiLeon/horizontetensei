...
Scene_LevelUpgrade.prototype.start = function () {
    ...
    var y = (this.calcWindowHeight(1, true)) + 56
    $gameActors._data.forEach((ator, index) => {
        ...
        var janela_levelUp = new Janela_LevelUp(new Rectangle(0, y + 72, Graphics.boxWidth, this.calcWindowHeight(4, true)), ator);
        this.addWindow(janela_levelUp);
        y += this.calcWindowHeight(5, true) + 32;
    });
};
...
Janela_LevelUp.prototype.drawItem = function (index) {
    ...
    this.resetTextColor();
    this.changePaintOpacity(this.isCommandEnabled(index));
    this.drawIcon(this.commandName(index).icon, rect.x, rect.y - 16)
    this.drawText(this.commandName(index).nome, rect.x, rect.y + 16, rect.width, 'left');
    this.drawText(this.commandName(index).atr, rect.x, rect.y - 16, rect.width, 'right');
};
